...
# from neratives_code
openai.api_key = "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"


def create_rules_from_prompt(ontology_file, rules_file):
    free_text_request = "write sparql selection query which extracts the targets which drove near a point of interest"
...
Your response should only include the code, without any explanations or annotations.
    ...
    prompt = prefix + ontology + rules + suffix
...
def run_nerratives(args, rdfox_server, narrative_selection_query=None):
    ...
    list_of_IRI.append('<https://oxfordsemantic.tech/RDFox/getting-started/')  # schema-free-elements
    ...
    if to_ins == 1:
        ...
        for j in range(len(poi_longs)):
            # print(j)
            res = insert_data('"' + str(j) + '" a poi:poi ;\n    lat:lat ' + str(poi_lats[j]) + ';\n    lon:lon ' + str(
                poi_longs[j]) +';\n    poi_name:poi_name ' + str(poi_name[j]) + ' .')
        # dpoi = query("SELECT ?p WHERE { ?p a poi:poi }")
        with open(args.trajectoeis_csv) as csvfile:
            ...
            for nc in range(num_of_chunks):
                ...
                for i, row in enumerate(spamreader):
                    if i > 0:
                        if len(str(row[-1])) > 0:
                            ...
                            t = query('SELECT ?n WHERE {"' + str(row[4]) + '" num_of_dets:num_of_dets ?n}')
                            # print('ttext', t.content)
                            if t.text == '?n\n':  # if its target id is new- insert a new target
                                insert_data('"' + row[4] + '" a target:target ;\n num_of_dets:num_of_dets 1 .')
                                insert_data('"' + str(row[-1]) + '" a detection:detection ;\n    lat:lat ' + row[
                                    8] + ';\n    lon:lon ' + row[7] + ';\n    time:time "' + str(
                                    r[1]) + '";\n    end_time:end_time "' + str(r1[
                                                                                    1]) + '";\n    serial_num:serial_num 1;\n    is_marginal:is_marginal "0" ;\n    detection_of:detection_of "' +
                                                  row[4] + '".')
                                ...
                                response = delete_data(
                                    '["' + row[4] + '", num_of_dets:num_of_dets, ' + t.text.split('\n')[1] + '] .')
                                res = insert_data('"' + row[4] + '" num_of_dets:num_of_dets ' + str(
                                    int(t.text.split('\n')[1]) + 1) + '.')
                                ...
                                if show_len_tar == 1:
                                    ti = query('SELECT ?n WHERE {"' + row[4] + '" num_of_dets:num_of_dets ?n.}')
                                    # print('ttext_after_insert', ti.content)
                                res = insert_data('"' + str(row[-1]) + '" a detection:detection ;\n   lat:lat ' + row[
                                    8] + ';\n   lon:lon ' + row[7] + ';\n   time:time "' + str(
                                    r[1]) + '";\n   end_time:end_time "' + str(
                                    r1[1]) + '";\n   serial_num:serial_num ' + str(int(t.text.split('\n')[
                                                                                           1]) + 1) + ';\n   is_marginal:is_marginal "0" ;\n   detection_of:detection_of "' +
                                                  row[4] + '".')
                ...
                d = query(
                    "SELECT ?d ?lon ?lat ?t WHERE { ?d a detection:detection . ?d lat:lat ?lat .?d lon:lon ?lon. ?d detection_of:detection_of ?t} GROUP BY (?d)")
                dlist = d.text.split('\n')
                lats = []
                longs = []
                tars = []
                ...
                for k in range(len(coorsx)):
                    res = insert_data(de[coorsx[k]] + ' near_poi:near_poi "' + str(p[coorsy[k] - 1]) + '".')
                # dis=euclidean_distances
                # print('number of detections:', len(dlist))
                # print('number of poi:', len(p))
                neardets = 1
                if neardets == 1:
                    ...
                    for ii in range(len(lats)):
                        ...
                        for jj in range(ii, len(lats)):
                            if tars[ii] != tars[jj]:
                                dis[ii, jj] = np.sqrt((lats[ii] - lats[jj]) **
                                                      2 + (longs[ii] - longs[jj]) ** 2)
                    coorsx, coorsy = np.where(dis < near_par)
                    for k in range(len(coorsx)):
                        if np.mod(k, 100) == 0:
                            # print(k, len(coorsx))
                            if de[coorsx[k]] != de[coorsy[k]]:
                                response11 = query('INSERT {' + de[coorsx[k]] + ' near_on_time:near_on_time ' + de[
                                    coorsy[k]] + '} WHERE {' + de[coorsx[k]] + ' detection_of:detection_of ?t1. ' + de[
                                                       coorsy[
                                                           k]] + ' detection_of:detection_of ?t2  . FILTER (?t1!=?t2).' +
                                                   de[coorsx[k]] + ' time:time ?bt1. ' + de[
                                                       coorsy[k]] + ' time:time ?bt2. ' + de[
                                                       coorsx[k]] + ' time:end_time ?et1. ' + de[coorsy[
                                    k]] + ' time:end_time ?et2. FILTER(?bt1<=?et2). FILTER(?bt2<=?et1)}')
                ...
                # plot all detections
                # plot all pois
                d = query("SELECT ?p ?lon ?lat WHERE { ?p a poi:poi . ?p lat:lat ?lat .?p lon:lon ?lon. } GROUP BY{?p}")
                ...
                # Narrative 1: Locate all of the detections that are near poi.
                if 0 in operated_narratives:
                    print('begins query 0')
                    d0 = query(
                        "SELECT ?d ?lon ?lat WHERE { ?d detection_of:detection_of ?p. ?d lat:lat ?lat. ?d lon:lon ?lon.} GROUP BY (?d)")
                    beg5 = time.time()
                    time4 = beg5 - beg4
                ...
                if 1 in operated_narratives:
                    ...
                    d1 = query(
                        "SELECT ?d ?lon ?lat WHERE { ?d a detection:detection. ?d near_poi:near_poi ?p. ?d lat:lat ?lat. ?d lon:lon ?lon.} GROUP BY (?d)")
                    beg5 = time.time()
                    time4 = beg5 - beg4
                ...
                if 2 in operated_narratives:
                    ...
                    i = 0
                    gg = query("SELECT ?t WHERE { ?t a target:target . ?t num_of_dets:num_of_dets ?n .FILTER(?n>2).}")
                    beg6 = time.time()
                    time5 = beg6 - beg5
                ...
                if 3 in operated_narratives:
                    ...
                    d3 = query(
                        "SELECT ?d ?lon ?lat WHERE { ?d lat:lat ?lat. ?d lon:lon ?lon. ?d near_on_time:near_on_time ?d2.} GROUP BY (?d)")
                ...
                if 4 in operated_narratives:
                    ...
                    d4 = query(
                        "SELECT ?d ?lon ?lat WHERE { ?d lat:lat ?lat. ?d lon:lon ?lon. ?d stop_on_time:stop_on_time ?d2.}  GROUP BY (?d)")
                ...
                if 5 in operated_narratives:
                    ...
                    d5 = query(
                        'SELECT ?t ?lon ?lat WHERE {?d detection_of:detection_of ?t.  ?d is_part_of_convoy:is_part_of_convoy "1". ?d lat:lat ?lat. ?d lon:lon ?lon.}')
                ...
                if 6 in operated_narratives:
                    ...
                    tracks_near_poi = extract_data1(d4)
                    # plt.plot(blon4, blat4, 'm.')
                    beg9 = time.time()
                    time6 = beg9 - beg8

                    print('Tracks near POI', tracks_near_poi)
                    print('time it took ', time6)
                ...
                if is_prob == 1:
                    ...
                    d6 = query('SELECT ?d ?p ?o WHERE {?d is_part_of_convoy:is_part_of_convoy ?o. ?d  ?p ?o}')
                    d7 = query('SELECT ?d ?p ?o WHERE {?d stop_on_time:stop_on_time ?o. ?d  ?p ?o}')
    ...
    plt.figure()
    plt.plot(np.arange(300, 3300, 300), timess, 'b', label='streaming and reasoning')
    plt.plot(np.arange(300, 3300, 300), timess22, 'g', label='quering')
    if is_prob == 1:
        plt.plot(np.arange(300, 3300, 300), timess11, 'r', label='probabilistic calc')
    plt.ylabel('time')
    plt.xlabel('num_of_points')
    plt.legend()
